...
export const Rikei2021: Problem[] = [
  ...
  },
  {
    ...
    "category": "理系",
    "content": {
      "question": "曲線 \\( y=\\log(1+\\cos x) \\) の \\( 0\\le x\\le \\frac{\\pi}{2} \\) の部分の長さを求めよ．",
      "hints": [
        "1. 曲線の長さは \\( L=\\int_{0}^{\\frac{\\pi}{2}} \\sqrt{1+\\left(\\frac{dy}{dx}\\right)^2}\\,dx \\) で求める．",
        "2. \\( y=\\log(1+\\cos x) \\) の微分をチェーンルールで求める．",
        "3. 半角公式 \\( 1+\\cos x=2\\cos^2\\frac{x}{2} \\) を利用して整理する．",
        "4. 変数置換 \\( u=\\sin\\frac{x}{2} \\) を用いて積分を簡単にする．"
      ],
      "solution": "答えは \\( 2\\log(1+\\sqrt{2}) \\) である．",
      "detailedSolution": [
        "【1. 曲線の長さの公式】",
        "曲線 \\( y=\\log(1+\\cos x) \\) の長さ \\( L \\) は",
        "\\[ L=\\int_{0}^{\\frac{\\pi}{2}} \\sqrt{1+\\left(\\frac{dy}{dx}\\right)^2}\\,dx \\] と表される．",
        "",
        "【2. 微分の計算】",
        "与えられた関数 \\( y=\\log(1+\\cos x) \\) の微分は、チェーンルールにより",
        "\\[ \\frac{dy}{dx}=\\frac{1}{1+\\cos x}(-\\sin x)=-\\frac{\\sin x}{1+\\cos x} \\] となる．",
        "よって、",
        "\\[ \\left(\\frac{dy}{dx}\\right)^2=\\frac{\\sin^2 x}{(1+\\cos x)^2} \\] である．",
        "",
        "【3. 被積分関数の整理】",
        "曲線の長さの公式内の平方根の中身は",
        "\\[ 1+\\left(\\frac{dy}{dx}\\right)^2 = 1+\\frac{\\sin^2 x}{(1+\\cos x)^2}=\\frac{(1+\\cos x)^2+\\sin^2 x}{(1+\\cos x)^2}. \\]",
        "分子は",
        "\\[ (1+\\cos x)^2+\\sin^2 x = 1+2\\cos x+\\cos^2 x+\\sin^2 x = 1+2\\cos x+1 = 2(1+\\cos x) \\] となるので、",
        "\\[ 1+\\left(\\frac{dy}{dx}\\right)^2 = \\frac{2(1+\\cos x)}{(1+\\cos x)^2}=\\frac{2}{1+\\cos x}. \\]",
        "したがって、",
        "\\[ L=\\int_{0}^{\\frac{\\pi}{2}} \\sqrt{\\frac{2}{1+\\cos x}}\\,dx = \\sqrt{2}\\int_{0}^{\\frac{\\pi}{2}} \\frac{dx}{\\sqrt{1+\\cos x}}. \\]",
        "",
        "【4. 半角公式の利用】",
        "半角公式 \\( 1+\\cos x=2\\cos^2\\frac{x}{2} \\) により",
        "\\[ \\sqrt{1+\\cos x}=\\sqrt{2\\cos^2\\frac{x}{2}}=\\sqrt{2}\\cos\\frac{x}{2} \\] （\\( 0\\le x\\le \\frac{\\pi}{2} \\) では \\( \\cos\\frac{x}{2}\\ge0 \\)）となる．",
        "これを用いると、",
        "\\[ L=\\sqrt{2}\\int_{0}^{\\frac{\\pi}{2}} \\frac{dx}{\\sqrt{2}\\cos\\frac{x}{2}}=\\int_{0}^{\\frac{\\pi}{2}} \\frac{dx}{\\cos\\frac{x}{2}}. \\]",
        "",
        "【5. 置換 \\( u=\\sin\\frac{x}{2} \\) を用いた積分】",
        "ここで \\( u=\\sin\\frac{x}{2} \\) と置く．両辺微分すると",
        "\\[ du=\\frac{1}{2}\\cos\\frac{x}{2}\\,dx \\quad\\Rightarrow\\quad dx=2\\,\\frac{du}{\\cos\\frac{x}{2}}. \\]",
        "よって、",
        "\\[ \\frac{dx}{\\cos\\frac{x}{2}} = \\frac{2\\,du}{\\cos^2\\frac{x}{2}}. \\]",
        "また、\\( \\cos^2\\frac{x}{2}=1-\\sin^2\\frac{x}{2}=1-u^2 \\) であるから、",
        "\\[ \\frac{dx}{\\cos\\frac{x}{2}}=\\frac{2\\,du}{1-u^2}. \\]",
        "積分区間は、\\( x=0 \\) で \\( u=0 \\) 、\\( x=\\frac{\\pi}{2} \\) で \\( u=\\sin\\frac{\\pi}{4}=\\frac{\\sqrt{2}}{2} \\) となる．したがって、",
        "\\[ L=2\\int_{0}^{\\frac{\\sqrt{2}}{2}} \\frac{du}{1-u^2}. \\]",
        "",
        "【6. 積分の計算】",
        "積分 \\( \\int \\frac{du}{1-u^2} \\) は部分分数分解により",
        "\\[ \\int \\frac{du}{1-u^2}=\\frac{1}{2}\\log\\left|\\frac{1+u}{1-u}\\right|+C \\] と求まる．",
        "よって、",
        "\\[ L=2\\cdot\\frac{1}{2}\\left[ \\log\\left|\\frac{1+u}{1-u}\\right| \\right]_{0}^{\\frac{\\sqrt{2}}{2}} = \\left[ \\log\\left|\\frac{1+u}{1-u}\\right| \\right]_{0}^{\\frac{\\sqrt{2}}{2}}. \\]",
        "評価すると、",
        "\\[ L=\\log\\left(\\frac{1+\\frac{\\sqrt{2}}{2}}{1-\\frac{\\sqrt{2}}{2}}\\right). \\]",
        "",
        "【7. 式の整理】",
        "通分して整理すると",
        "\\[ \\frac{1+\\frac{\\sqrt{2}}{2}}{1-\\frac{\\sqrt{2}}{2}}=\\frac{2+\\sqrt{2}}{2-\\sqrt{2}}. \\]",
        "有理化すると、",
        "\\[ \\frac{2+\\sqrt{2}}{2-\\sqrt{2}}=\\frac{(2+\\sqrt{2})^2}{4-2}=\\frac{(2+\\sqrt{2})^2}{2}. \\]",
        "実際、\\( (2+\\sqrt{2})^2=6+4\\sqrt{2} \\) なので、",
        "\\[ \\frac{(2+\\sqrt{2})^2}{2}=3+2\\sqrt{2}. \\]",
        "さらに、\\( 3+2\\sqrt{2}=(1+\\sqrt{2})^2 \\) であるから、",
        "\\[ L=\\log\\left((1+\\sqrt{2})^2\\right)=2\\log(1+\\sqrt{2}). \\]",
        "",
        "【結論】",
        "以上より、曲線の長さは",
        "\\[ \\boxed{2\\log(1+\\sqrt{2})} \\] である．"
      ]
    }
  }
];
